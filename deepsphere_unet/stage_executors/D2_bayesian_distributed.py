...
class ModelTrainer(BayesianDeepSphereModelExecutor):
    def __init__(self, cfg: DictConfig, rank) -> None:
        ...
        self.show_mem_stats = cfg.model.deepsphere.train_bayesian.show_mem_stats
    ...
    def load_model_weights(self, model):
        with self.name_tracker.set_context("epoch", "best"):
            logger.info(f"Attemtping to load model from {self.in_model.path}")
            try:
...
def dist_run(rank, world_size, cfg, logger):
    ...
    loss_fn = trainer.heteroscedastic_loss
    optimizer = optim.Adam(ddp_model.parameters(), lr=trainer.lr)
...
class DistributedBayesianExecutor(BayesianDeepSphereModelExecutor):
    def __init__(self, cfg: DictConfig) -> None:
        super().__init__(cfg, stage_str="train")

        self.cfg = cfg
        self.world_size = cfg.model.deepsphere.train_bayesian.n_gpus
